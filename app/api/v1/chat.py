...
from app.schemas.chat_schema import (
    ...
    MessageResponse, ConversationUpdate, ChatAssessmentRequest, ChatAssessmentResponse
)
...
@router.post("/tutor/chat-assessment")
async def assess_chat_answer(
        request: "ChatAssessmentRequest",
        current_user: Dict[str, Any] = Depends(get_current_user),
        db: AsyncSession = Depends(get_db)
):
    """
    Assess a student's answer to a follow-up assessment question.
    
    This endpoint is called when a student responds to the assessment question
    generated by the AI tutor. Mastery is only updated on correct answers.
    """
    from app.schemas.chat_schema import ChatAssessmentRequest, ChatAssessmentResponse
    from app.services.ai_tutor_service import ai_tutor_service
    from app.services.ai_service import ConversationMessage
    ...
    try:
        ...
        # Get conversation history for context
        conversation_messages = await _get_conversation_messages(request.conversation_id, db)
        messages = [
            ConversationMessage(role=msg["role"], content=msg["content"])
            for msg in conversation_messages
        ]

        # Assess the student's answer using AI
        assessment = await ai_tutor_service.assess_student_understanding(
        ...
        # Check for positive indicators in the AI assessment
        assessment_text = str(assessment.get("assessment", "")).lower()
        was_correct = any(indicator in assessment_text for indicator in [
            "correct", "good understanding", "well done", "excellent",
            "demonstrates understanding", "accurate", "right"
        ]) and not any(indicator in assessment_text for indicator in [
            "incorrect", "wrong", "misunderstanding", "not quite", "needs improvement"
        ])
        
        # Parse concepts demonstrated vs missed
        concepts_demonstrated = []
        concepts_missed = []
        
        for concept in request.expected_concepts:
            if concept.lower() in request.student_answer.lower() or concept.lower() in assessment_text:
                concepts_demonstrated.append(concept)
            else:
                concepts_missed.append(concept)
        
        # If most concepts are demonstrated, consider it correct
        if len(concepts_demonstrated) >= len(request.expected_concepts) * 0.6:
            was_correct = True
        
        # Update mastery based on assessment (only if correct)
        mastery_updated = False
        new_mastery_percentage = None
        
        if request.topic:
            try:
                mastery_service = MasteryCalculationService(db)
                await mastery_service.update_mastery_from_chat_assessment(
                    user_id=str(user.id),
                    subject_tag=request.topic,
                    was_correct=was_correct,
                    expected_concepts=request.expected_concepts
                )
                mastery_updated = True
                
                # Get updated mastery percentage
                from sqlalchemy import select, and_
                query = select(SubjectMastery.mastery_percentage).where(
                    and_(
                        SubjectMastery.user_id == user.id,
                        SubjectMastery.subject_tag == request.topic
                    )
                )
                result = await db.execute(query)
                mastery_pct = result.scalar_one_or_none()
                if mastery_pct:
                    new_mastery_percentage = float(mastery_pct)
                    
            except Exception as e:
                logger.warning(f"Failed to update mastery for topic {request.topic}: {e}")
        
        # Generate follow-up suggestion if answer was incorrect
        follow_up_suggestion = None
        if not was_correct and concepts_missed:
            follow_up_suggestion = (
                f"Let's review these concepts: {', '.join(concepts_missed)}. "
                "Would you like me to explain them in more detail?"
            )
        
        # Generate feedback
        if was_correct:
            feedback = (
                f"Great job! You correctly demonstrated understanding of: "
                f"{', '.join(concepts_demonstrated) if concepts_demonstrated else 'the key concepts'}. "
                "Your mastery has been updated."
            )
        else:
            feedback = (
                f"Good effort! You showed understanding of {', '.join(concepts_demonstrated) if concepts_demonstrated else 'some aspects'}. "
                f"Consider reviewing: {', '.join(concepts_missed) if concepts_missed else 'the related concepts'}."
            )
        
        logger.info(
            f"Chat assessment for user {user.id}: topic={request.topic}, "
            f"correct={was_correct}, mastery_updated={mastery_updated}"
        )
        ...
        return {
            ...
            "new_mastery_percentage": new_mastery_percentage,
        ...
        logger.error(f"Chat assessment failed: {e}")
        raise HTTPException(status_code=500, detail="Failed to assess answer")
